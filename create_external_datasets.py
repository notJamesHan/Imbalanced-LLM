...
def main():
    ...
    temp_output = "dataset-generation-" + datetime.datetime.now().strftime(
        "%Y%m%d-%H%M%S"
    )
    output_dir = (
        Path("/content/drive/MyDrive/Colab Notebooks/TabLLM/datasets_serialized")
        / temp_output
    )
    if not args.debug:
        os.mkdir(output_dir)
    ...
    # This part is the most important part of the code.
    # eval() will find the variable with the name of the string and assign it to "template" variable.
    # For example, if dataset_name is "income", then eval will find the variable "template_income" and assign it to "template" variable.
    # To find more variables, go to helper/external_datasets_variables.py
    # Written by: James
    ...
    notes = [
        NoteGenerator.clean_note(note_generator.substitute(r))
    ...
    print(
        f"Only consider dataset range between {start} and {end} (total: {old_size_notes})"
    )
    ...
    if not args.debug:
        ...
        logger.info(
            f"\tn={len(dataset)}, feats={dataset.num_columns}, labels={dict(Counter(dataset['label']))}"
        )
...
def load_train_validation_test(dataset_name, data_dir):
    ...
    def train_validation_test_split(data):
        ...
        data_valid, data_test = train_test_split(
            data_test, test_size=0.50, shuffle=False
        )
    ...
    if dataset_name == "creditg":
        ...
        dataset_train, dataset_valid, dataset_test = train_validation_test_split(
            dataset
        )
        ...
        dataset = pd.DataFrame(
            loadarff(data_dir / "jungle_chess_2pcs_raw_endgame_complete.arff")[0]
        )
        dataset = byte_to_string_columns(dataset)
        dataset.rename(columns={"class": "label"}, inplace=True)
        ...
        dataset_train = pd.read_csv(
            data_dir / "adult.data", names=columns, na_values=["?", " ?"]
        )
        ...
        assert (
            len(dataset_train) + len(dataset_valid) + len(dataset_test) == original_size
        )
        ...
        dataset = pd.read_csv(
            data_dir / "house-votes-84.data", names=columns, na_values=["?"]
        )
        original_size = len(dataset)
        ...
        dataset = pd.read_csv(
            data_dir / "winequality-red.csv", names=columns, skiprows=[0]
        )
        original_size = len(dataset)
    ...
    assert (
        dataset_name in dataset_specs.keys()
        and len(dataset.columns) == dataset_specs[dataset_name]
    )
